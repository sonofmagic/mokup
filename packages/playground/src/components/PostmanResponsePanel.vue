...
<template>
  <section class="flex min-h-[240px] flex-1 flex-col rounded-2xl border shadow-sm border-pg-border bg-pg-surface-card">
    ...
    <div class="flex-1 min-h-0 overflow-auto p-4">
      <pre
        v-if="activeTab === 'body'"
        class="min-h-[140px] rounded-xl border p-4 text-xs border-pg-border bg-pg-surface-strong text-pg-text"
        data-testid="playground-response"
      >{{ responseBody }}</pre>
      <div
        v-else-if="activeTab === 'headers'"
        class="min-h-[140px] rounded-xl border p-4 text-sm border-pg-border bg-pg-surface-strong text-pg-text-muted"
      >
