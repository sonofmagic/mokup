...
const field = tv({
  base: 'flex flex-col text-pg-text-muted',
  variants: {
    dense: {
      true: 'gap-1',
      false: 'gap-1.5',
...
const label = tv({
  base: 'text-pg-text-muted',
  variants: {
    dense: {
      true: 'text-[0.55rem] tracking-[0.1em]',
      false: 'text-[0.65rem] tracking-[0.08em]',
    },
  },
  defaultVariants: {
    dense: false,
...
const labelClasses = computed(() => label({ dense: props.dense }))
